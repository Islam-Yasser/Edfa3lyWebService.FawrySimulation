...
import com.efa3lyservices.edfa3lysevices.controller.DonationsController.OrganizationsFactory;
...
public class ServiceController {
    ...
    @GetMapping(value = "/services/mobile/{Service}/{id}/{phoneNum}/{servProvider}/{PayMethod}/{money}")
                                ...
                                @PathVariable("phoneNum") String phoneNum )
    {
       ...
       User user =this.customer.getUser(id);
        Services s=new ServiceFactory().createService(Service);
        ...
        if(x.substring(0,1).equalsIgnoreCase("1")){
            return x+" "+this.customer.addTransaction(Service,money,id,paymethod)+ss;
        }else return x;
    }
    @GetMapping (value ="/donations/{schools}/{schoola}/{money}/{id}/{payMethod}")
    public String addDonations(@PathVariable("schools") String DonationF,@PathVariable("schoola")String ServiceN,@PathVariable("money")double money
        ...
        User user=this.customer.getUser(id);
        OrganizationsFactory Of= (OrganizationsFactory) this.DF.getType(DonationF);
    ...
    @GetMapping(value = "/Receipts/{landline}/{phone}/{duration}/{money}/{id}/{payMethod}")
